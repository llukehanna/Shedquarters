...
/** The ESPN-style standings row. #1 is cardinal and wears the Shed of the Table badge. */
export function RankRow({
  ...
  return (
    <Link
      href={href}
      className={`mb-[5px] flex min-h-12 items-center overflow-hidden rounded-[7px] pr-3 ${first ? 'cardinal-panel min-h-[54px]' : 'surface'} ${provisional ? 'opacity-60' : ''}`}
    >
      <span
        className={`flex w-11 self-stretch items-center justify-center border-r font-display text-[21px] font-extrabold ${first ? 'border-gold/35 text-white' : 'border-gold/12 text-gold'}`}
      >
        {rank}
      </span>
      <span className="ml-3 min-w-0 flex-1">
        <span className="flex items-center gap-2">
          ...
          {first && <Pill tone="gold">Shed of the Table</Pill>}
          ...
          {streak && (
            <span
              className={`font-display text-[12px] font-extrabold uppercase ${
                first ? 'text-cream' : streak.result === 'W' ? 'text-up' : 'text-down'
              }`}
            >
              {/* text-up and text-down are both too close to the #1 row's
                  ...
                  so `first` swaps either for text-cream instead. */}
              {formatStreak(streak)}
            </span>
          )}
        </span>
        ...
        {movement !== null && (
          <span
            className={`font-mono text-[11px] font-bold ${
              first ? 'text-cream' : movement > 0 ? 'text-up' : movement < 0 ? 'text-down' : 'text-muted'
            }`}
          >
            {/* None of text-up/text-down/text-faint reliably clear 4.5:1 on
                ...
                checked first and always swaps to text-cream there. Off the
